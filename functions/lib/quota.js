...
export async function initUserQuota(db, userId, projectId) {
  ...
  await db.prepare(`
    INSERT OR IGNORE INTO user_quotas (user_id, project_id, plan_id, credits_monthly, credits_used_this_month, credits_reset_at, credits_bonus, credits_bonus_total)
    VALUES (?, ?, 'free', 10, 0, ?, 3, 3)
  `).bind(userId, projectId, resetAt).run();
}
...
export async function checkQuota(db, userId, projectId) {
  ...
  if (quota.credits_reset_at && new Date(quota.credits_reset_at) <= now) {
    ...
    const plan = await db.prepare(
      'SELECT credits_monthly FROM subscription_plans WHERE id = ?'
    ).bind(quota.plan_id).first();
    ...
    `).bind(monthlyCredits, nextReset, userId, projectId).run();
...
export async function deductCredit(db, userId, jobId, projectId) {
  ...
  // 记录使用日志
  await db.prepare(`
    INSERT INTO usage_logs (user_id, job_id, credits_used, source, status, project_id) VALUES (?, ?, 1, ?, 'success', ?)
